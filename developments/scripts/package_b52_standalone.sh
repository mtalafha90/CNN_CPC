...
#
#   DATA_ROOT=/path/to/rsna-knee-abnormality-detection ./run.sh [epochs]
set -euo pipefail
cd "$(dirname "$0")"
...
DATA_ROOT=/path/to/rsna-knee-abnormality-detection ./run.sh 8
```
...
```bash
DATA_ROOT=/path/to/data ./run.sh 8 --preflight-only
```
