"""B30 fixed-E2 training on the exact historical B20 gradient surface.

Single intervention versus B29/B20 family: the complementary series summary uses
B20's current learned Q/K/V, output projection and LayerNorm affine parameters as
detached deterministic operators. Only a new 768-D query and zero-init 768-D gate
are added. The historical B20 attention pool executes first and unchanged.

All supervision, data surface, encoder, crop, optimizer, augmentation, loader seed,
scheduler horizon and fixed-E2 endpoint remain frozen. Expert labels are not read.
"""
...
B30_TRAIN_VERSION = "1.0.0"
B30_EXPERIMENT = "B30_zero_gated_projected_complementary_series_summary"
B30_VARIANT = "b20_plus_zero_init_projected_complementary_attention_fixed_e2_v1"
B30_FIXED_EPOCHS = 2
B30_EXPECTED_STUDIES = 3120
B30_EXPECTED_SERIES = 17475
B30_EXPECTED_CELLS = 14123
B30_EXPECTED_POS = 6871
B30_EXPECTED_NEG = 7252
B30_MAX_HARD_HOURS = 8.25
B30_MIN_RESERVE_MINUTES = 30.0
B30_ATTENTION_AUDIT_BATCHES_PER_EPOCH = 64


def _validate_attention_audit(state: dict, *, epoch: int) -> None:
    if int(state.get("series_count", 0)) <= 0:
        raise RuntimeError(f"B30 E{epoch} attention audit recorded zero series")
    bounded = (
    ...
    for key in bounded:
        ...
        if value is None or not math.isfinite(float(value)) or not (0.0 <= float(value) <= 1.0):
            raise RuntimeError(f"B30 E{epoch} invalid attention audit metric {key}={value}")
    for key in ("effective_residual_norm_ratio_mean", "effective_residual_norm_ratio_max"):
        value = state.get(key)
        if value is None or not math.isfinite(float(value)) or float(value) < 0.0:
            raise RuntimeError(f"B30 E{epoch} invalid residual audit metric {key}={value}")
...
def train_b30(
    ...
    *,
    ...
    out_root: str | Path = "runs/b30_projected_complementary_series_pool",
) -> Path:
    ...
    max_hours = min(float(config.get("runtime_budget_hours", 8.5)), B30_MAX_HARD_HOURS)
    reserve_minutes = max(
        float(config.get("runtime_reserve_minutes", 10.0)),
        B30_MIN_RESERVE_MINUTES,
    )
    ...
    if len(study_uids) != B30_EXPECTED_STUDIES:
        raise RuntimeError(f"B30 requires {B30_EXPECTED_STUDIES} studies")
    ...
    if (active_cells, positive_cells, negative_cells) != (
        ...
        raise RuntimeError("B30 B6 supervision surface drifted from historical B20")
    ...
    if any(bool(gold_by_uid[uid_to_row[uid]]) for uid in study_uids):
        raise RuntimeError("expert-gold study unexpectedly entered B30 gradients")
    ...
    if series_policy.get("series_summary", {}).get("series_signature_sha256") != B13_SERIES_SIGNATURE:
        raise ValueError("B30 requires the frozen B12/B13 series policy")
    ...
    if series_summary.get("series_signature_sha256") != B13_SERIES_SIGNATURE:
        raise ValueError("B30 reconstructed series surface does not match frozen B13 SHA-256")
    if int(series_summary.get("eligible_recognized_plane_series", -1)) != B30_EXPECTED_SERIES:
        raise ValueError("B30 requires the exact 17,475-series B20 gradient surface")
    if series_summary.get("viability_passed") is not True:
        raise ValueError("B30 reconstructed series surface failed viability")
    ...
    if expected_batches != 1560:
        raise RuntimeError("B30 exact B20 surface must yield 1,560 batches per epoch")
    ...
    spec = b30_model_spec(config, normalize_input=True)
    model = build_b30_model(spec, pretrained_weights=False)
    ...
    if state_initial["new_parameter_count"] != B30_EXPECTED_NEW_PARAMETERS:
        raise RuntimeError("B30 new parameter count changed")
    if state_initial["query_parameter_count"] != B30_EXPECTED_QUERY_PARAMETERS:
        raise RuntimeError("B30 complementary query parameter count changed")
    if state_initial["gate_parameter_count"] != B30_EXPECTED_GATE_PARAMETERS:
        raise RuntimeError("B30 complementary gate parameter count changed")
    if state_initial["gate_raw_max_abs"] != 0.0 or state_initial["gate_effective_max_abs"] != 0.0:
        raise RuntimeError("B30 complementary residual gate must start at exactly zero")
    ...
    if not head_params or any(p.requires_grad for p in model.encoder.parameters()):
        raise RuntimeError("B30 frozen-encoder contract failed")
    ...
    print(
        f"[B30] studies={len(study_uids)} series={B30_EXPECTED_SERIES} "
        f"cells={active_cells} | new_params={B30_EXPECTED_NEW_PARAMETERS} "
        f"(query={B30_EXPECTED_QUERY_PARAMETERS}, gate={B30_EXPECTED_GATE_PARAMETERS}) "
        f"| fixed E{B30_FIXED_EPOCHS}"
    )
    print(
        "[B30] token=A+tanh(g)*(C2-A) | A=B20 MHA token | C2=new query through "
        "detached current B20 Q/K/V+out+LN | complementary dropout=0"
    )
    print(
        f"[B30] attention audit first {B30_ATTENTION_AUDIT_BATCHES_PER_EPOCH} "
        "frozen-loader batches/epoch | no expert labels"
    )
    print(
        f"[B30] hard_runtime={max_hours:.2f} h | reserve={reserve_minutes:.0f} min"
    )

    history: list[dict] = []
    attention_audit_history: list[dict] = []
    for epoch in range(1, B30_FIXED_EPOCHS + 1):
        ...
        for batch_index, batch in enumerate(loader, start=1):
            if not budget.can_start(120.0):
                raise RuntimeError("B30 runtime budget expired before complete fixed-E2")
            model.enable_attention_audit(
                batch_index <= B30_ATTENTION_AUDIT_BATCHES_PER_EPOCH,
                reset=False,
            )
            ...
            if any(p.grad is not None for p in model.encoder.parameters()):
                raise RuntimeError("B30 detected an encoder gradient")
            ...
            if gate_grad is None or not torch.isfinite(gate_grad).all():
                raise RuntimeError("B30 complementary gate gradient missing or non-finite")
            if query_grad is None or not torch.isfinite(query_grad).all():
                raise RuntimeError("B30 complementary query gradient missing or non-finite")
        ...
        if encoder_state_sha256(model.encoder) != encoder_sha_initial:
            raise RuntimeError("B30 encoder changed despite freezing")
        full = (
            steps == expected_batches
            and seen_studies == B30_EXPECTED_STUDIES
            and seen_series == B30_EXPECTED_SERIES
            and seen_cells == B30_EXPECTED_CELLS
            and seen_pos == B30_EXPECTED_POS
            and seen_neg == B30_EXPECTED_NEG
        )
        if not full:
            raise RuntimeError(f"B30 epoch {epoch} did not complete the frozen surface")
        if not gate_grad_seen:
            raise RuntimeError(f"B30 epoch {epoch} never produced a nonzero gate gradient")
        if not query_grad_seen:
            raise RuntimeError(f"B30 epoch {epoch} never coupled the complementary query to the loss")
        ...
        history.append(row)
        print(
            f"[B30] E{epoch} loss={row['loss']:.10f} | {epoch_seconds/60:.1f} min "
            f"| gate|max|={state['gate_effective_max_abs']:.6f} "
            f"| gate|mean|={state['gate_effective_mean_abs']:.6f} "
            f"| JS={attention_state['js_divergence_normalized_mean']:.4f} "
            f"| top1={attention_state['top1_slice_agreement']:.3f} "
    ...
    if encoder_sha_final != encoder_sha_initial:
        raise RuntimeError("B30 encoder fingerprint changed")

    out = Path(out_root)
    out.mkdir(parents=True, exist_ok=True)
    checkpoint = out / "b30_model.pt"
    state_final = model.complementary_state()
    payload = {
        "experiment": B30_EXPERIMENT,
        "variant": B30_VARIANT,
        "b30_training_version": B30_TRAIN_VERSION,
        "b30_residual_version": B30_RESIDUAL_VERSION,
        "fixed_endpoint": True,
        "selected_epoch": B30_FIXED_EPOCHS,
        "completed_epochs": B30_FIXED_EPOCHS,
        ...
        "attention_audit_batches_per_epoch": B30_ATTENTION_AUDIT_BATCHES_PER_EPOCH,
    ...
    audit = {
        ...
        "fixed_epoch": B30_FIXED_EPOCHS,
        "training_studies": B30_EXPECTED_STUDIES,
        "series_instances": B30_EXPECTED_SERIES,
        "supervision_cells": B30_EXPECTED_CELLS,
        "positive_cells": B30_EXPECTED_POS,
        "negative_cells": B30_EXPECTED_NEG,
        "new_parameter_count": B30_EXPECTED_NEW_PARAMETERS,
        "query_parameter_count": B30_EXPECTED_QUERY_PARAMETERS,
        "gate_parameter_count": B30_EXPECTED_GATE_PARAMETERS,
        "shared_projection_gradient_from_complementary_branch": False,
        "complementary_dropout": 0.0,
        "complementary_pool_initial": state_initial,
        "complementary_pool_final": state_final,
        "attention_audit_batches_per_epoch": B30_ATTENTION_AUDIT_BATCHES_PER_EPOCH,
        "attention_audit_history": attention_audit_history,
    ...
    (out / "attention_audit.json").write_text(
        json.dumps(attention_audit_history, indent=2), encoding="utf-8"
    )
...
def load_b30_checkpoint(path: str | Path, *, device: torch.device | str = "cpu"):
    payload = torch.load(path, map_location="cpu", weights_only=False)
    if payload.get("experiment") != B30_EXPERIMENT or payload.get("variant") != B30_VARIANT:
        raise ValueError("not a B30 fixed-E2 projected-complementary checkpoint")
    if payload.get("fixed_endpoint") is not True or int(payload.get("completed_epochs", -1)) != B30_FIXED_EPOCHS:
        raise ValueError("B30 checkpoint must be the complete fixed-E2 endpoint")
    if int(payload.get("gold_studies_used_in_gradient", -1)) != 0:
        raise ValueError("B30 checkpoint does not certify zero gold gradients")
    if payload.get("expert_checkpoint_selection") is not False:
        raise ValueError("B30 checkpoint unexpectedly used expert checkpoint selection")
    ...
    if not initial_sha or initial_sha != final_sha:
        raise ValueError("B30 encoder fingerprint changed")
    final_state = payload.get("complementary_pool_final") or {}
    if int(final_state.get("new_parameter_count", -1)) != B30_EXPECTED_NEW_PARAMETERS:
        raise ValueError("B30 checkpoint new-parameter contract changed")

    model = build_b30_model(payload["model_spec"], pretrained_weights=False)
    model.load_state_dict(payload["model_state"], strict=True)
    freeze_encoder(model)
    if encoder_state_sha256(model.encoder) != initial_sha:
        raise ValueError("B30 reconstructed encoder fingerprint mismatch")
    return model.to(device), payload


def main() -> None:
    parser = argparse.ArgumentParser("B30 zero-gated projected complementary attention")
    ...
    parser.add_argument("--out-root", default="runs/b30_projected_complementary_series_pool")
