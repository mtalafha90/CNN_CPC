"""Paired reused-expert diagnostic for historical B20 versus fixed-E2 B27.1.

B27.1 is trained on the full historical 3,120-study B20 gradient surface, so the
623-study weak-v2 partition is not a holdout. The 58-study expert surface is
heavily reused development data and selected historical B20 checkpoints. This
comparison is descriptive/post-hoc and cannot independently promote B27.1.
"""
...
from .b27_1_training import B27_1_EXPERIMENT, B27_1_VARIANT, load_b27_1_checkpoint
...
def _shared_spec(spec: dict) -> dict:
    ...
    out.pop("b27_1_routing_version", None)
    out.pop("b27_1_route_parameter_count", None)
    out.pop("b27_1_unknown_or_discordant_sequence_bias", None)
    return out


def evaluate_b27_1_reused_gold(
    config: dict,
    *,
    b20_checkpoint: str | Path,
    b27_1_checkpoint: str | Path,
    out_root: str | Path,
    ...
    candidate_model, candidate_payload = load_b27_1_checkpoint(
        b27_1_checkpoint, device=runtime.device
    )
    b20_model.eval()
    ...
    if str(b20_payload.get("encoder_sha256_initial", "")) != str(
        ...
        raise RuntimeError("B20 and B27.1 do not share the same frozen encoder fingerprint")
    if b20_payload.get("crop_focus_policy") != candidate_payload.get("crop_focus_policy"):
        raise RuntimeError("B20 and B27.1 crop policies differ")
    if _shared_spec(b20_payload.get("model_spec") or {}) != _shared_spec(
        candidate_payload.get("model_spec") or {}
    ):
        raise RuntimeError("B20 and B27.1 shared model specifications differ")
    ...
    print("[B27.1 gold] predicting historical B20 on reused 58-study expert surface")
    b20_uids, b20_pred = predict_b12_1(b20_model, expert["loader"], runtime)
    print("[B27.1 gold] predicting fixed-E2 B27.1 on the same surface")
    ...
    if [str(x) for x in cand_uids] != expected_uids:
        raise RuntimeError("B27.1 expert prediction order changed")

    truth = expert["truth"]
    seed = int(config.get("seed", 2026))
    b20_eval = bootstrap_macro_auc(
        truth, b20_pred, n_bootstrap=n_bootstrap, seed=seed + 27_101
    )
    cand_eval = bootstrap_macro_auc(
        truth, cand_pred, n_bootstrap=n_bootstrap, seed=seed + 27_102
    )
    paired = compare_runs(
        truth,
        b20_pred,
    ...
    result = {
        "evaluation_version": B27_1_GOLD_EVAL_VERSION,
        ...
        "weak_v2_used": False,
        "weak_v2_reason": (
            "B27.1 trains on all 3,120 historical B20 weak-supervision studies, including "
        ...
        "b27_1_checkpoint_selection_used_expert_surface": False,
        "promotion_allowed_from_this_result_alone": False,
        "b20": b20_eval.to_dict(),
        "b27_1": cand_eval.to_dict(),
        "raw_b27_1_minus_b20": raw_delta,
        "paired_b27_1_minus_b20": paired,
        "per_target": per_target,
        "routing_final": candidate_payload.get("routing_final"),
        "paired_metadata_audit": candidate_payload.get("paired_metadata_audit"),
        ...
        "interpretation_guardrail": (
            ...
            "remains the independent performance signal. Do not tune B27.1 routing, "
            "metadata categories or endpoint from this result."
        ),
    }
    ...
    for j, target in enumerate(TARGETS):
        ...
        frame[f"{target}__B27_1"] = cand_pred[:, j]
    ...
    print("\n" + "=" * 76)
    print("B27.1 REUSED EXPERT DEVELOPMENT RESULT -- NOT INDEPENDENT VALIDATION")
    print("=" * 76)
    print(f"B20     macro AUC : {b20_eval.macro_auc:.10f}")
    print(f"B27.1   macro AUC : {cand_eval.macro_auc:.10f}")
    print(f"raw delta          : {raw_delta:+.10f}")
    print(
        "paired 95% CI     : "
        f"[{paired['ci_lower']:+.10f}, {paired['ci_upper']:+.10f}]"
    )
    print(f"P(B27.1 > B20)     : {paired['probability_b_better']:.4f}")
    print("\nPer-target")
    print("-" * 64)
    print(f"{'Target':18s} {'B20':>10s} {'B27.1':>10s} {'Delta':>10s}")
    for target, row in per_target.items():
        print(
            f"{target:18s} {row['b20']:10.4f} {row['b27_1']:10.4f} "
            f"{row['b27_1_minus_b20']:+10.4f}"
        )
...
def main() -> None:
    ap = argparse.ArgumentParser("B27.1 paired reused-expert development evaluation")
    ap.add_argument("--config", default="config/current_model.yaml")
    ap.add_argument("--data-root", required=True)
    ap.add_argument("--b20-checkpoint", required=True)
    ap.add_argument("--b27-1-checkpoint", required=True)
    ap.add_argument("--out-root", default="runs/b27_1_pathology_routing/reused_gold_eval")
    ...
    evaluate_b27_1_reused_gold(
        config,
        b20_checkpoint=args.b20_checkpoint,
        b27_1_checkpoint=args.b27_1_checkpoint,
