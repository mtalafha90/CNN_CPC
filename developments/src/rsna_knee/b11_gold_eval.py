"""Development-only frozen gold evaluation for B11 teacher-student completion."""
...
from .b11_teacher_student import B11_EXPERIMENT, B11_VARIANT, load_b11_checkpoint
...
def evaluate_b11_gold(
    config: dict,
    *,
    checkpoint: str | Path,
    out_root: str | Path = "runs/b11_teacher_student/gold_eval",
) -> dict:
    runtime = resolve_runtime(config)
    print(runtime.describe())
    model, checkpoint_payload = load_b11_checkpoint(checkpoint, device=runtime.device)
    if int(checkpoint_payload.get("completed_epochs", -1)) != 4:
        raise ValueError("B11-v1 gold evaluation requires the completed four-epoch checkpoint")
    history = checkpoint_payload.get("history", [])
    if len(history) != 4 or not all(bool(row.get("full_coverage")) for row in history):
        raise ValueError("B11-v1 gold evaluation requires four complete full-coverage epochs")
    ...
    if len(gold) != 58 or gold[TARGETS].isna().any().any():
        raise ValueError("B11-v1 requires the complete 58-study gold development surface")
    ...
    if offsets != (-1, 0, 1):
        raise ValueError("B11-v1 gold TTA is frozen at [-1,0,1]")
    ...
    loader = DataLoader(
        ...
        shuffle=False,
        **runtime.loader_kwargs(seed=int(config.get("seed", 2026)) + 11_500_000),
    )
    pred_uids, prediction = predict_b7(model, loader, runtime)
    if pred_uids != uids:
        raise RuntimeError("B11 gold prediction order changed unexpectedly")
    ...
    payload.update({
        "experiment": "B11_gold_development_evaluation",
        "checkpoint": str(Path(checkpoint).resolve()),
        "b11_variant": B11_VARIANT,
        "training_experiment": B11_EXPERIMENT,
        ...
        "gold_labels_used_in_b11_gradient": False,
        "gold_labels_used_for_b11_early_stopping": False,
        ...
        "initialization": "B5 competition-only image-report encoder (same as B7.1 student initialization)",
        "single_scientific_change_vs_b7_1": "conservative frozen B7.1-teacher pseudo-supervision on B6-unsupervised cells",
        "metadata_repair": metadata_stats,
        "interpretation": (
            "development estimate; pseudo-cell selection used no gold labels, but the B7.1 teacher "
            "was itself retained after prior development on the same 58-study surface"
        ),
    })
...
def main() -> None:
    parser = argparse.ArgumentParser("rsna-knee-b11-eval")
    parser.add_argument("--config", required=True)
    parser.add_argument("--data-root", default=None)
    parser.add_argument("--checkpoint", required=True)
    parser.add_argument("--out-root", default="runs/b11_teacher_student/gold_eval")
    args = parser.parse_args()
    config = _read_config(args.config)
    if args.data_root:
        config = dict(config)
        config["data_root"] = args.data_root
    payload = evaluate_b11_gold(config, checkpoint=args.checkpoint, out_root=args.out_root)
