"""B27 fixed-E2 training on the exact historical B20 weak-supervision surface.

The only model change is the zero-initialised 84-parameter pathology-specific
metadata routing bias in :mod:`rsna_knee.b27_pathology_routing`.

Everything else remains B20: 3,120 studies, 14,123 B6 cells, 17,475 series,
B16 report-aligned frozen encoder, 90% post-resize crop, optimizer, augmentation,
loader seed, and the five-epoch cosine scheduler horizon.  Training stops at E2
without reading expert labels.

A conservative 8.25-hour hard budget with a 30-minute internal reserve is used
for this experiment.  The observed B20-family E2 runtime is about one hour, so
this guard is intentionally much stricter than the user-supplied 9-hour
competition ceiling.
"""
...
from .b27_pathology_routing import (
...
B27_TRAIN_VERSION = "1.0.0"
B27_EXPERIMENT = "B27_pathology_specific_metadata_routing"
B27_VARIANT = "b20_plus_zero_init_pathology_metadata_attention_bias_fixed_e2_v1"
B27_FIXED_EPOCHS = 2
B27_EXPECTED_STUDIES = 3120
B27_EXPECTED_SERIES = 17475
B27_EXPECTED_CELLS = 14123
B27_EXPECTED_POS = 6871
B27_EXPECTED_NEG = 7252
B27_MAX_HARD_HOURS = 8.25
B27_MIN_RESERVE_MINUTES = 30.0
...
def _routing_norms(model) -> dict:
    params = torch.cat(
        [
            ...
            model.route_fluid_bias.detach().float().reshape(-1).cpu(),
            model.route_fat_bias.detach().float().reshape(-1).cpu(),
        ]
    )
    return {
...
def train_b27(
    ...
    *,
    ...
    out_root: str | Path = "runs/b27_pathology_routing",
) -> Path:
    ...
    # Keep a platform-sized safety margin even if a looser config is supplied.
    max_hours = min(float(config.get("runtime_budget_hours", 8.5)), B27_MAX_HARD_HOURS)
    reserve_minutes = max(
        float(config.get("runtime_reserve_minutes", 10.0)),
        B27_MIN_RESERVE_MINUTES,
    )
    ...
    if len(study_uids) != B27_EXPECTED_STUDIES:
        raise RuntimeError(f"B27 requires {B27_EXPECTED_STUDIES} studies")
    ...
    if (active_cells, positive_cells, negative_cells) != (
        ...
        raise RuntimeError("B27 B6 supervision surface drifted from historical B20")

    gold_by_uid = torch.as_tensor(
        gold_mask(train).to_numpy(dtype=bool), dtype=torch.bool
    )
    ...
    if any(bool(gold_by_uid[uid_to_row[uid]]) for uid in study_uids):
        raise RuntimeError("expert-gold study unexpectedly entered B27 gradients")
    ...
    if series_policy.get("series_summary", {}).get("series_signature_sha256") != B13_SERIES_SIGNATURE:
        raise ValueError("B27 requires the frozen B12/B13 series policy")
    ...
    if series_summary.get("series_signature_sha256") != B13_SERIES_SIGNATURE:
        raise ValueError("B27 reconstructed series surface does not match frozen B13 SHA-256")
    if int(series_summary.get("eligible_recognized_plane_series", -1)) != B27_EXPECTED_SERIES:
        raise ValueError("B27 requires the exact 17,475-series B20 gradient surface")
    if series_summary.get("viability_passed") is not True:
        raise ValueError("B27 reconstructed series surface failed viability")
    ...
    if expected_batches != 1560:
        raise RuntimeError("B27 exact B20 surface must yield 1,560 batches per epoch")
    ...
    spec = b27_model_spec(config, normalize_input=True)
    model = build_b27_model(spec, pretrained_weights=False)
    ...
    if route_initial["parameter_count"] != B27_ROUTE_PARAMETER_COUNT:
        raise RuntimeError("B27 routing parameter count changed")
    if route_initial["max_abs"] != 0.0:
        raise RuntimeError("B27 routing must be exactly zero-initialised")
    ...
    if not head_params or any(p.requires_grad for p in model.encoder.parameters()):
        raise RuntimeError("B27 frozen-encoder contract failed")
    ...
    print(
        f"[B27] studies={len(study_uids)} series={B27_EXPECTED_SERIES} "
        f"cells={active_cells} | routing_params={B27_ROUTE_PARAMETER_COUNT} "
        f"| fixed E{B27_FIXED_EPOCHS}"
    )
    print(
        f"[B27] hard_runtime={max_hours:.2f} h | reserve={reserve_minutes:.0f} min "
        "| expert labels=not read"
    )

    history: list[dict] = []
    for epoch in range(1, B27_FIXED_EPOCHS + 1):
        ...
        for batch in loader:
            if not budget.can_start(120.0):
                raise RuntimeError("B27 runtime budget expired before a complete fixed-E2 pass")
            ...
            if any(p.grad is not None for p in model.encoder.parameters()):
                raise RuntimeError("B27 detected an encoder gradient")
            if clip > 0:
        ...
        if encoder_state_sha256(model.encoder) != encoder_sha_initial:
            raise RuntimeError("B27 encoder changed despite freezing")
        full = (
            steps == expected_batches
            and seen_studies == B27_EXPECTED_STUDIES
            and seen_series == B27_EXPECTED_SERIES
            and seen_cells == B27_EXPECTED_CELLS
            and seen_pos == B27_EXPECTED_POS
            and seen_neg == B27_EXPECTED_NEG
        )
        if not full:
            raise RuntimeError(f"B27 epoch {epoch} did not complete the exact frozen surface")
        ...
        history.append(row)
        print(
            f"[B27] E{epoch} loss={row['loss']:.10f} | {epoch_seconds/60:.1f} min "
    ...
    if encoder_sha_final != encoder_sha_initial:
        raise RuntimeError("B27 encoder fingerprint changed")

    out = Path(out_root)
    out.mkdir(parents=True, exist_ok=True)
    checkpoint = out / "b27_model.pt"
    payload = {
        "experiment": B27_EXPERIMENT,
        "variant": B27_VARIANT,
        "b27_training_version": B27_TRAIN_VERSION,
        "b27_routing_version": B27_ROUTING_VERSION,
        "fixed_endpoint": True,
        "selected_epoch": B27_FIXED_EPOCHS,
        "completed_epochs": B27_FIXED_EPOCHS,
    ...
    audit = {
        ...
        "fixed_epoch": B27_FIXED_EPOCHS,
        "training_studies": B27_EXPECTED_STUDIES,
        "series_instances": B27_EXPECTED_SERIES,
        "supervision_cells": B27_EXPECTED_CELLS,
        "positive_cells": B27_EXPECTED_POS,
        "negative_cells": B27_EXPECTED_NEG,
        "routing_parameter_count": B27_ROUTE_PARAMETER_COUNT,
        "routing_initial": route_initial,
        "routing_final": model.routing_tables(),
...
def load_b27_checkpoint(path: str | Path, *, device: torch.device | str = "cpu"):
    payload = torch.load(path, map_location="cpu", weights_only=False)
    if payload.get("experiment") != B27_EXPERIMENT or payload.get("variant") != B27_VARIANT:
        raise ValueError("not a B27 fixed-E2 pathology-routing checkpoint")
    if payload.get("fixed_endpoint") is not True or int(payload.get("completed_epochs", -1)) != B27_FIXED_EPOCHS:
        raise ValueError("B27 checkpoint must be the complete fixed-E2 endpoint")
    if int(payload.get("gold_studies_used_in_gradient", -1)) != 0:
        raise ValueError("B27 checkpoint does not certify zero gold gradients")
    if payload.get("expert_checkpoint_selection") is not False:
        raise ValueError("B27 checkpoint unexpectedly used expert checkpoint selection")
    ...
    if not initial_sha or initial_sha != final_sha:
        raise ValueError("B27 encoder fingerprint changed")

    model = build_b27_model(payload["model_spec"], pretrained_weights=False)
    model.load_state_dict(payload["model_state"], strict=True)
    freeze_encoder(model)
    if encoder_state_sha256(model.encoder) != initial_sha:
        raise ValueError("B27 reconstructed encoder fingerprint mismatch")
    return model.to(device), payload


def main() -> None:
    parser = argparse.ArgumentParser("B27 pathology-specific metadata routing")
    ...
    parser.add_argument("--out-root", default="runs/b27_pathology_routing")
