"""B14 training: B13 ImageNet protocol with the full B12 slice-token memory.

Single scientific change versus B13:
    B13: 16 slice tokens / real series -> one learned series token -> study Transformer
    B14: retain all K x 16 slice tokens -> study Transformer -> pathology queries

The ImageNet ConvNeXt-Tiny protocol, frozen B12 all-series surface, B6 supervision,
optimizer, augmentation, four-epoch schedule, TTA and bootstrap contract remain
unchanged. Shared trainable parameters follow the same seeded construction order
as B13 before ImageNet encoder weights are loaded.
"""
...
B14_VARIANT = "b14_imagenet_b6_full_slice_tokens_v1"
B14_EXPERIMENT = "B14_imagenet_full_slice_tokens"
B14_INITIALIZATION = "torchvision:convnext_tiny:IMAGENET1K_V1"
B14_INPUT_NORMALIZATION = "imagenet_mean_std"
B14_AGGREGATION = "all_real_series_x_16_slice_tokens_v1"
B14_SERIES_SIGNATURE = "5c4bb1c52294e45f9e83274c5c07d198dc54811c49b96111b7c8439bd7bcd376"
...
def _require_close(config: dict, key: str, expected: float) -> None:
    ...
    if not np.isclose(value, expected, atol=1e-12, rtol=0):
        raise ValueError(f"B14-v1 freezes {key}={expected}; got {value}")


def _require_b14_contract(config: dict) -> None:
    """Freeze B14 as the full-slice-token aggregation test versus B13."""
    _require_frozen_policy(config)
    if str(config.get("b14_experiment_name", B14_EXPERIMENT)) != B14_EXPERIMENT:
        raise ValueError(f"B14 experiment name must remain {B14_EXPERIMENT!r}")
    if str(config.get("b14_aggregation", B14_AGGREGATION)) != B14_AGGREGATION:
        raise ValueError(f"B14 aggregation must remain {B14_AGGREGATION!r}")
    if not bool(config.get("allow_external_pretrained", False)):
        raise ValueError("B14 requires allow_external_pretrained=true")
    if not bool(config.get("pretrained", False)):
        raise ValueError("B14 requires pretrained=true")
    if bool(config.get("b12_use_physical_scale", False)):
        raise ValueError("B14 freezes legacy resize; B10 physical normalization is disabled")
    ...
    if tuple(int(x) for x in config.get("b7_train_gap_choices", [1, 2])) != (1, 2):
        raise ValueError("B14-v1 freezes b7_train_gap_choices=[1,2]")
    if tuple(int(x) for x in config.get("b7_eval_tta_offsets", [-1, 0, 1])) != (-1, 0, 1):
        raise ValueError("B14-v1 freezes b7_eval_tta_offsets=[-1,0,1]")


def _load_imagenet_encoder_state(model) -> None:
    """Load the same ImageNet ConvNeXt-Tiny state used by B13."""
    ...
    *,
    ...
    return {
        ...
        "single_scientific_change_vs_b13": (
            "remove B13 learned one-token-per-series compression and retain every K x 16 "
            "slice token in the study Transformer memory"
        ),
        "supervision_policy": {
            ...
            "target_balancing": "unchanged B13 B6-derived multipliers",
        },
...
def train_b14(
    ...
    *,
    ...
    out_root: str | Path = "runs/b14_imagenet_full_tokens",
) -> Path:
    ...
    print(f"[B14] initialization={B14_INITIALIZATION}")
    print(f"[B14] input_normalization={B14_INPUT_NORMALIZATION}")
    print(f"[B14] aggregation={B14_AGGREGATION}")
    ...
    if (
        ...
        raise ValueError("B14 must retain the exact B13 B6 training surface")
    ...
    if series_summary.get("series_signature_sha256") != B14_SERIES_SIGNATURE:
        raise ValueError("B14 reconstructed series surface does not match the frozen B12 SHA-256")
    if frozen_summary.get("series_signature_sha256") != B14_SERIES_SIGNATURE:
        raise ValueError("B14 supplied series policy is not the frozen B12 policy")
    if int(series_summary.get("eligible_recognized_plane_series", -1)) != 17475:
        raise ValueError("B14 requires the frozen 17,475-series B12/B13 surface")
    if series_summary.get("viability_passed") is not True:
        raise ValueError("B14 reconstructed series surface no longer passes viability")
    ...
    # Build the B12 full-token architecture on the exact same seeded path used by
    # the B13 shared parameters, then replace only the encoder with the same
    # ImageNet state. The only intended model difference is the aggregation path.
    spec = b12_model_spec(config, normalize_input=True)
    spec["b14_aggregation"] = B14_AGGREGATION
    ...
    checkpoint_path = outdir / "b14_model.pt"
    ...
    policy_payload = {
        "experiment": B14_EXPERIMENT,
        "variant": B14_VARIANT,
        "status": "B14-v1 recipe frozen before first gold evaluation",
        "single_scientific_change_vs_b13": (
            "B13 one learned token per real series -> all K x 16 slice tokens retained "
            "through the study Transformer and pathology-query attention"
        ),
        "student_initialization": B14_INITIALIZATION,
    ...
    for epoch in range(epochs):
        if epoch_times and not budget.can_start(float(np.median(epoch_times)) * 1.20):
            print("[budget] stopping B14 before next epoch")
            break
        ...
        for batch in loader:
            if not budget.can_start(120.0):
                ...
                print("[budget] stopping B14 batches before wall-clock reserve")
                break
        ...
        if steps == 0:
            raise RuntimeError("B14 completed no training batches")
        scheduler.step()
    ...
    if len(history) != 4 or not all(
        ...
        print(
            "[warning] B14 did not complete the frozen four-pass study/series contract; "
            "do not run gold evaluation"
        )
    return checkpoint_path


def load_b14_checkpoint(
    checkpoint: str | Path,
    *,
    ...
    if not isinstance(payload, dict) or payload.get("variant") != B14_VARIANT:
        raise ValueError(f"not a {B14_VARIANT} checkpoint")
    if payload.get("initialization") != B14_INITIALIZATION:
        raise ValueError("B14 checkpoint does not certify the frozen ImageNet initialization")
    if payload.get("input_normalization") != B14_INPUT_NORMALIZATION:
        raise ValueError("B14 checkpoint does not certify the frozen ImageNet normalization")
    if payload.get("aggregation") != B14_AGGREGATION:
        raise ValueError("B14 checkpoint does not certify the full-slice-token aggregation")
    if payload.get("external_pretrained") is not True:
        raise ValueError("B14 checkpoint does not certify external ImageNet pretraining")
    if int(payload.get("gold_studies_used_in_gradient", -1)) != 0:
        raise ValueError("B14 checkpoint does not certify zero gold-gradient studies")
    spec = payload.get("model_spec")
    state = payload.get("model_state")
    if not isinstance(spec, dict) or not isinstance(state, dict):
        raise ValueError("B14 checkpoint missing model_spec/model_state")
    model = build_b12_model(spec)
...
def main() -> None:
    parser = argparse.ArgumentParser("rsna-knee-b14")
    parser.add_argument("--config", required=True)
    parser.add_argument("--data-root", default=None)
    parser.add_argument("--b6-root", required=True)
    parser.add_argument("--series-policy", required=True)
    parser.add_argument("--out-root", default="runs/b14_imagenet_full_tokens")
