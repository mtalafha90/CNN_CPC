...
from .training import contract, load_encoder
...
def fine_contract(root, p, rsna, runtime):
    ...
    return contract(root, p, runtime, "finetune") | {
        ...
        "gold_studies_in_gradient": 0, "external_labels_used": True,
        "prior_native_supervision_epochs": p["config"]["supervised_epochs"],
        "native_model_sha256": meta["native_model_sha256"]}
...
def train(root, *, device="cuda", workers=0):
    ...
    if (out/"complete.json").exists():
        ...
        print("[B58 full] supervised training already COMPLETE", flush=True)
        return out/"final.pt"
    ...
    print(f"[B58 full] finetune {runtime.describe()}; {state.describe()}", flush=True)
    for epoch in range(state.start_epoch, c["epochs"]+1):
        ...
        for batch, items in enumerate(loader, 1):
            ...
            if batch % 100 == 0:
                print(f"[B58 full] E{epoch} {batch}/{len(loader)} loss={total/batch:.5f}", flush=True)
        ...
        print(f"[B58 full] E{epoch} macroAUC={scores['macro_auc']:.6f}", flush=True)
    ...
    print("[B58 full] supervised training: COMPLETE", flush=True)
    return out/"final.pt"
