"""Dual-GPU hidden-test inference for the frozen B39 five-offset endpoint.

This module changes execution infrastructure only.  B39 remains the exact frozen
B37 fixed-E2 checkpoint averaged over the predeclared five symmetric centre
offsets [-2, -1, 0, +1, +2].  Studies are deterministically sharded by test-row
index across two identical model replicas, one per visible Kaggle T4.  Each
worker processes a complete study independently, so there is no cross-study
batching and no change to model semantics or probability aggregation.
"""
...
from .b37_highres_sparse_submission import (
    ...
    _require_b37_checkpoint_contract,
    _submission_budget,
...
B39_DUALGPU_EXECUTION_VERSION = "b39_hidden_dual_t4_study_shards_v1"
B39_DUALGPU_COUNT = 2


def _release_worker_memory(device: torch.device) -> None:
    """Release tensors from one completed study without altering the model."""
    gc.collect()
    if device.type == "cuda":
...
def _load_replica(checkpoint_path: Path, base_path: Path, device: torch.device):
    """Load and verify one byte-identical frozen B37/B39 model replica."""
    model, payload = load_b37_checkpoint(
        checkpoint_path,
        base_checkpoint=base_path,
        device=device,
        expected_version=B37_VERSION,
        expected_experiment=B37_EXPERIMENT,
        checkpoint_label="B39/B37",
    )
...
def _infer_shard(
    *,
    ...
    """Run one deterministic B39 study shard on one CUDA device."""
        ...
        for local_position, index in enumerate(indices):
            if durations:
                ...
                projected = projected_remaining_seconds(
                    ...
                    safety_factor=B39_TIMING_SAFETY_FACTOR,
                )
                ...
                if projected > available:
                    raise RuntimeError(
                        f"B39 dual-GPU shard {rank} cannot finish inside the "
            ...
            if volumes.ndim != 6 or int(volumes.shape[0]) != len(B39_TTA_OFFSETS):
                raise RuntimeError(
                    f"B39 dual-GPU TTA volume shape changed for row {index}: "
                    f"{tuple(volumes.shape)}"
                )
            if position.ndim != 3 or int(position.shape[0]) != len(B39_TTA_OFFSETS):
                raise RuntimeError("B39 dual-GPU slice-position TTA shape changed")
            ...
            if not torch.isfinite(probability).all():
                raise RuntimeError(
                    f"B39 dual-GPU produced non-finite probabilities for row {index}"
                )
            ...
            if completed % 10 == 0 or completed == len(indices):
                ...
                print(
                    f"[B39 dual submit gpu{rank}] {completed}/{len(indices)} "
...
def generate_b39_submission_dual_gpu(
    config: dict,
    *,
    data_root: str | Path,
    checkpoint: str | Path,
    ...
    """Run exact frozen B39 five-offset inference on two independent GPUs."""
    ...
    if visible < B39_DUALGPU_COUNT:
        print(
            f"[B39 dual submit] only {visible} CUDA device(s) visible; "
            "falling back to the audited single-GPU B39 path",
            flush=True,
        )
        output = generate_b39_submission(
            settings,
            data_root=root,
        ...
        if output is None:
            raise RuntimeError("B39 single-GPU fallback unexpectedly returned no output")
        return output

    if runtime.num_workers != 0 or runtime.pin_memory:
        raise RuntimeError("B39 dual-GPU submission requires workers=0 and pin_memory=False")
    print(runtime.describe(), flush=True)
    print(
        f"[B39 dual submit] using cuda:0 + cuda:1; exact TTA offsets="
        f"{list(B39_TTA_OFFSETS)}",
        flush=True,
    )
    ...
    if not checkpoint_path.is_file():
        raise FileNotFoundError(f"B39/B37 checkpoint is missing: {checkpoint_path}")
    if not base_path.is_file():
        raise FileNotFoundError(f"B39 base checkpoint is missing: {base_path}")

    # Sequential deserialization avoids two simultaneous host-RAM spikes.
    ...
    if payload0.get("encoder_sha256_final") != payload1.get("encoder_sha256_final"):
        raise RuntimeError("B39 dual-GPU replicas do not describe the same B37 endpoint")
    ...
    if missing:
        raise ValueError(
            f"B39 dual-GPU submission found {len(missing)} test study/studies "
    ...
    datasets = [
        B37HighResSparseDataset(
            ...
            center_offsets=B39_TTA_OFFSETS,
        )
        for _ in range(B39_DUALGPU_COUNT)
    ]
    shards = [
        list(range(rank, len(uids), B39_DUALGPU_COUNT))
        for rank in range(B39_DUALGPU_COUNT)
    ]
    ...
    with ThreadPoolExecutor(max_workers=B39_DUALGPU_COUNT) as pool:
        futures = [
            pool.submit(
                _infer_shard,
                rank=rank,
    ...
    if [row[0] for row in rows] != list(range(len(uids))):
        raise RuntimeError("B39 dual-GPU output row indices are incomplete or duplicated")
    uid_rows = [row[1] for row in rows]
    if uid_rows != uids:
        raise RuntimeError("B39 dual-GPU submission changed StudyInstanceUID order")
    ...
    manifest = {
        ...
        "version": B39_VERSION,
        "execution_version": B39_DUALGPU_EXECUTION_VERSION,
        "execution_only_change": True,
        "gpu_count": B39_DUALGPU_COUNT,
        "study_sharding": "test-row index modulo 2",
        "cross_study_batching": False,
        "parent_checkpoint_experiment": payload0.get("experiment"),
        "parent_checkpoint_version": payload0.get("version"),
        ...
        "fixed_endpoint": bool(payload0.get("fixed_endpoint")),
        ...
        "governance": (
            "Exact frozen B39 inference-only endpoint. Dual-GPU study sharding "
            "changes execution wall time only; B37 checkpoint, 448 preprocessing, "
            "five offsets [-2,-1,0,1,2], sigmoid probability averaging, sparse-MIL, "
...
__all__ = [
    "B39_DUALGPU_EXECUTION_VERSION",
    "generate_b39_submission_dual_gpu",
]
