"""Paired reused-expert diagnostic for historical B20 versus fixed-E2 B29.

B29 trains on the full 3,120-study historical B20 gradient surface, so weak-v2
...
hoc and cannot independently promote B29.
"""
...
from .b29_training import B29_EXPERIMENT, B29_VARIANT, load_b29_checkpoint
...
def _shared_spec(spec: dict) -> dict:
    ...
    out.pop("b29_residual_version", None)
    out.pop("b29_new_parameter_count", None)
    out.pop("b29_complementary_scorer", None)
    out.pop("b29_complementary_summary", None)
    out.pop("b29_gate_constraint", None)
    out.pop("b29_stochastic_path", None)
    return out


def evaluate_b29_reused_gold(
    config: dict,
    *,
    ...
    candidate_model, candidate_payload = load_b29_checkpoint(
        b29_checkpoint, device=runtime.device
    )
    b20_model.eval()
    ...
    if str(b20_payload.get("encoder_sha256_initial", "")) != str(
        ...
        raise RuntimeError("B20 and B29 do not share the same frozen encoder fingerprint")
    if b20_payload.get("crop_focus_policy") != candidate_payload.get("crop_focus_policy"):
        raise RuntimeError("B20 and B29 crop policies differ")
    if _shared_spec(b20_payload.get("model_spec") or {}) != _shared_spec(
        candidate_payload.get("model_spec") or {}
    ):
        raise RuntimeError("B20 and B29 shared model specifications differ")
    ...
    print("[B29 gold] predicting historical B20 on reused 58-study expert surface")
    b20_uids, b20_pred = predict_b12_1(b20_model, expert["loader"], runtime)
    print("[B29 gold] predicting fixed-E2 B29 on the same surface")
    ...
    if [str(x) for x in cand_uids] != expected_uids:
        raise RuntimeError("B29 expert prediction order changed")

    truth = expert["truth"]
    seed = int(config.get("seed", 2026))
    b20_eval = bootstrap_macro_auc(
        truth, b20_pred, n_bootstrap=n_bootstrap, seed=seed + 29_001
    )
    cand_eval = bootstrap_macro_auc(
        truth, cand_pred, n_bootstrap=n_bootstrap, seed=seed + 29_002
    )
    paired = compare_runs(
        truth,
        b20_pred,
    ...
    result = {
        "evaluation_version": B29_GOLD_EVAL_VERSION,
        ...
        "weak_v2_used": False,
        "weak_v2_reason": (
            "B29 trains on all 3,120 historical B20 weak-supervision studies, including "
            "the 623 UIDs used as weak-v2 only in partial-surface experiments."
        ),
        "b20_control_was_selected_on_this_expert_surface": True,
        "b29_checkpoint_selection_used_expert_surface": False,
        "promotion_allowed_from_this_result_alone": False,
        "b20": b20_eval.to_dict(),
        "b29": cand_eval.to_dict(),
        "raw_b29_minus_b20": raw_delta,
        "paired_b29_minus_b20": paired,
        ...
        "interpretation_guardrail": (
            ...
            "remains the independent performance signal. Do not tune the B29 query, gate, "
            "endpoint, or target-specific behavior from this result."
        ),
    }
    ...
    for j, target in enumerate(TARGETS):
        ...
        frame[f"{target}__B29"] = cand_pred[:, j]
    ...
    print("\n" + "=" * 76)
    print("B29 REUSED EXPERT DEVELOPMENT RESULT -- NOT INDEPENDENT VALIDATION")
    print("=" * 76)
    print(f"B20   macro AUC : {b20_eval.macro_auc:.10f}")
    print(f"B29   macro AUC : {cand_eval.macro_auc:.10f}")
    print(f"raw delta        : {raw_delta:+.10f}")
    print(
        "paired 95% CI   : "
        f"[{paired['ci_lower']:+.10f}, {paired['ci_upper']:+.10f}]"
    )
    print(f"P(B29 > B20)     : {paired['probability_b_better']:.4f}")
    print("\nPer-target")
    print("-" * 64)
    print(f"{'Target':18s} {'B20':>10s} {'B29':>10s} {'Delta':>10s}")
    for target, row in per_target.items():
        print(
            f"{target:18s} {row['b20']:10.4f} {row['b29']:10.4f} "
            f"{row['b29_minus_b20']:+10.4f}"
        )
...
def main() -> None:
    ap = argparse.ArgumentParser("B29 paired reused-expert development evaluation")
    ...
    ap.add_argument("--out-root", default="runs/b29_complementary_series_pool/reused_gold_eval")
