"""Competition inference for the selected B20 crop-only checkpoint."""
...
def generate_b20_submission(
    ...
    *,
    ...
    if payload.get("variant") != B20_VARIANT or payload.get("experiment") != B20_EXPERIMENT:
        raise ValueError("checkpoint is not the selected B20 experiment")
    ...
    if missing:
        raise ValueError(f"B20 test inference found {len(missing)} study/studies with zero eligible series")

    offsets = tuple(int(x) for x in config.get("b7_eval_tta_offsets", [-1, 0, 1]))
    if offsets != (-1, 0, 1):
        raise ValueError("B20 submission freezes TTA at [-1,0,1]")
    ...
    for batch_index, batch in enumerate(loader):
        if batch_times:
            ...
            budget.require(projected, label="remaining B20 submission inference")
        else:
            budget.require(180.0, label="first B20 submission batch")
        ...
        if volumes.ndim != 7:
            raise RuntimeError(
                f"B20 submission expects [B,V,K,S,C,H,W], got {tuple(volumes.shape)}"
            )
        if int(volumes.shape[1]) != len(offsets):
            raise RuntimeError("B20 submission TTA view count changed")
    ...
    if uid_rows != uids:
        raise RuntimeError("B20 test inference changed StudyInstanceUID order")
    ...
    manifest = {
        "experiment": "B20_crop_only_submission_inference",
        "variant": B20_VARIANT,
...
def main() -> None:
    parser = argparse.ArgumentParser("rsna-knee-b20-submit")
    ...
    generate_b20_submission(config, checkpoint=args.checkpoint, out_path=args.out)
