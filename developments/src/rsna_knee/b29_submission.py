"""Competition inference for the frozen B29 complementary-series-pool candidate."""
...
@torch.no_grad()
def generate_b29_submission(
    config: dict,
    *,
    checkpoint: str | Path,
    out_path: str | Path = "submission_b29.csv",
) -> Path:
    ...
    model, payload = load_b29_checkpoint(checkpoint, device=runtime.device)
    if payload.get("variant") != B29_VARIANT or payload.get("experiment") != B29_EXPERIMENT:
        raise ValueError("checkpoint is not the frozen B29 experiment")
    ...
    if missing:
        raise ValueError(f"B29 test inference found {len(missing)} study/studies with zero eligible series")

    offsets = tuple(int(x) for x in config.get("b7_eval_tta_offsets", [-1, 0, 1]))
    if offsets != (-1, 0, 1):
        raise ValueError("B29 submission freezes TTA at [-1,0,1]")
    ...
    for batch_index, batch in enumerate(loader):
        if batch_times:
            ...
            budget.require(projected, label="remaining B29 submission inference")
        else:
            budget.require(180.0, label="first B29 submission batch")
        ...
        if volumes.ndim != 7:
            raise RuntimeError(
                f"B29 submission expects [B,V,K,S,C,H,W], got {tuple(volumes.shape)}"
            )
        if int(volumes.shape[1]) != len(offsets):
            raise RuntimeError("B29 submission TTA view count changed")
    ...
    if uid_rows != uids:
        raise RuntimeError("B29 test inference changed StudyInstanceUID order")
    ...
    manifest = {
        "experiment": "B29_frozen_candidate_submission_inference",
        "variant": B29_VARIANT,
        ...
        "expert_score_role": "reused post-hoc development evidence only; not checkpoint selection",
        "encoder_frozen": bool(payload.get("encoder_frozen")),
        "encoder_sha256": payload.get("encoder_sha256_final"),
        "complementary_pool_final": payload.get("complementary_pool_final"),
        ...
        "governance": (
            "Exact frozen B29 fixed-E2 candidate. Do not tune query, gate, endpoint, "
            "target-specific behavior, blending, or preprocessing from hidden evaluation."
        ),
        **sample_validation,
    }
    ...
    print("submission_sha256", submission_sha256)
    print(json.dumps(manifest, indent=2))
    return output


def main() -> None:
    parser = argparse.ArgumentParser("rsna-knee-b29-submit")
    parser.add_argument("--config", default="config/current_model.yaml")
    parser.add_argument("--data-root", required=True)
    parser.add_argument("--checkpoint", required=True)
    parser.add_argument("--out", default="submission_b29.csv")
    args = parser.parse_args()

    config = dict(_read_config(args.config))
    config["data_root"] = str(Path(args.data_root).resolve())
    generate_b29_submission(config, checkpoint=args.checkpoint, out_path=args.out)
