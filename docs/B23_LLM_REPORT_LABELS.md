...
**Output-budget truncation.** Grammar-constrained decoding emits well-formed JSON right up to the token limit and then stops, so a completion that runs out of budget fails as `Unterminated string starting at: line 61 column 19`. This happened on the first real run at `num_predict: 2048`: twelve verbatim evidence spans in Turkish, Dutch and Spanish tokenise far worse than English prose. Three things now prevent it — the output budget defaults to `4096` and auto-escalates when Ollama reports `done_reason == "length"`, evidence spans are capped at 120 characters in the prompt and again after parsing, and `extract_report` no longer retries a truncation, since greedy decoding is deterministic and an identical request produces an identical failure.
