...
def load_b5_encoder_payload(checkpoint: str | Path) -> dict:
    ...
    if payload.get("source") != SSL_SOURCE:
        raise ValueError(f"B7 requires B5 source={SSL_SOURCE!r}; got {payload.get('source')!r}")
    if payload.get("variant") != B5_VARIANT:
        raise ValueError(f"B7 requires B5 variant={B5_VARIANT!r}; got {payload.get('variant')!r}")
...
def make_b7_dataset_config(config: dict, root: str | Path, *, train: bool, tta_offsets: tuple[int, ...] = ()) -> DatasetConfig:
...
def b7_model_spec(config: dict, *, normalize_input: bool) -> dict:
    return {
        ...
        "n_streams": len(DUAL_STREAMS), "n_slices": int(config.get("b7_n_slices", 16)), "in_channels": 3,
        "image_size": int(config.get("b7_image_size", 224)), "triplet_gap": int(config.get("b7_triplet_gap", 1)),
        "dropout": float(config.get("b7_dropout", 0.25)), "normalize_input": bool(normalize_input),
...
def build_b7_model(spec: dict, *, encoder_state: dict | None = None) -> KneeMILNet:
    model = KneeMILNet(
        int(spec["n_streams"]), int(spec["n_slices"]), in_channels=int(spec.get("in_channels", 3)),
        pretrained_weights=False, normalize_input=bool(spec["normalize_input"]), dropout=float(spec["dropout"]),
        encoder_batch_size=int(spec["encoder_batch_size"]), gradient_checkpointing=bool(spec["gradient_checkpointing"]),
        transformer_layers=int(spec["transformer_layers"]), transformer_heads=int(spec["transformer_heads"]),
        transformer_ff_mult=float(spec["transformer_ff_mult"]), pathology_layers=int(spec["pathology_layers"]),
    )
...
def _checkpoint_payload(*, model, config, spec, history, b5_checkpoint, b5_payload, b6_root, b6_audit, supervision, target_multiplier, metadata_stats, budget) -> dict:
    return {
        "variant": B7_VARIANT, "source": SSL_SOURCE, "model_state": model.state_dict(), "encoder": model.encoder.state_dict(),
        "model_spec": spec, "config": config, "completed_epochs": len(history),
        "gold_studies_used_in_gradient": 0, "gold_studies_used_for_early_stopping": 0,
        "b6_gold_audit_informed_global_policy": True, "b5_checkpoint": str(b5_checkpoint.resolve()),
        "b5_variant": b5_payload.get("variant"), "b5_completed_epochs": b5_payload.get("completed_epochs"),
        "b6_root": str(b6_root.resolve()), "b6_version": b6_audit.get("b6_version"),
        "supervision_policy": {"min_confidence": B7_MIN_CONFIDENCE, "positive_target": B7_POSITIVE_TARGET,
            "negative_target": B7_NEGATIVE_TARGET, "positive_weight": B7_POSITIVE_WEIGHT,
            "negative_weight": B7_NEGATIVE_WEIGHT, "uncertain_weight": 0.0, "unmentioned_weight": 0.0,
        ...
        "supervision": supervision, "metadata_repair": metadata_stats, "history": history, "budget": budget.to_dict(),
    }


def train_b7(config: dict, *, b5_checkpoint: str | Path, b6_root: str | Path, out_root: str | Path = "runs/b7_weak_supervision") -> Path:
    ...
    budget = RuntimeBudget(max_hours=float(config.get("runtime_budget_hours", 8.5)), reserve_minutes=float(config.get("runtime_reserve_minutes", 10.0)))
    ...
    ds = KneeStudyDataset(uids, index, make_b7_dataset_config(config, root, train=True), targets=targets, weights=weights, train=True)
    batch_size = int(config.get("b7_batch_size", 2))
    if batch_size < 1:
        raise ValueError("b7_batch_size must be >=1")
    loader = DataLoader(ds, batch_size=batch_size, shuffle=True, drop_last=True, **runtime.loader_kwargs(seed=seed + 7_100_000))
    ...
    encoder_lr, head_lr = float(config.get("b7_encoder_lr", 1e-5)), float(config.get("b7_head_lr", 1e-4))
    encoder_params = list(model.encoder.parameters())
    head_params = [p for name, p in model.named_parameters() if not name.startswith("encoder.")]
    optimizer = torch.optim.AdamW([{"params": encoder_params, "lr": encoder_lr}, {"params": head_params, "lr": head_lr}], weight_decay=float(config.get("b7_weight_decay", 1e-4)))
    epochs, max_batches = int(config.get("b7_epochs", 4)), int(config.get("b7_max_batches_per_epoch", 500))
    if epochs < 1 or max_batches < 1:
        raise ValueError("b7_epochs and b7_max_batches_per_epoch must be >=1")
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=max(1, epochs), eta_min=float(config.get("b7_min_lr", 1e-6)))
    scaler, clip = make_scaler(runtime), float(config.get("b7_grad_clip", 1.0))
    ...
    policy_payload = {"experiment": "B7", "variant": B7_VARIANT, "status": "training recipe frozen before B7 gold evaluation",
        "b5_initialization": str(b5_path.resolve()), "b5_variant": b5_payload.get("variant"), "b6_root": str(Path(b6_root).resolve()),
        "b6_version": b6_audit.get("b6_version"), "b6_policy": b6_policy, "gold_labels_in_training_loss": False,
        "gold_labels_for_early_stopping": False, "b6_gold_audit_informed_global_policy": True,
        "fixed_epochs": epochs, "max_batches_per_epoch": max_batches,
        "supervision_policy": {"min_confidence": B7_MIN_CONFIDENCE, "positive_target": B7_POSITIVE_TARGET,
            "negative_target": B7_NEGATIVE_TARGET, "positive_weight": B7_POSITIVE_WEIGHT,
            "negative_weight": B7_NEGATIVE_WEIGHT, "uncertain_weight": 0.0, "unmentioned_weight": 0.0},
        "target_balancing": {target: float(target_multiplier[j]) for j, target in enumerate(TARGETS)}, "model_spec": spec}
    ...
    for epoch in range(epochs):
        ...
        for batch_index, batch in enumerate(loader):
            ...
            scaler.update()
            active = weight > 0
            loss_sum += float(loss.item()); steps += 1; study_draws += int(volumes.shape[0]); active_cells += int(active.sum().item())
            positive_cells += int((active & (target > 0.5)).sum().item()); negative_cells += int((active & (target < 0.5)).sum().item())
        epoch_seconds = time.monotonic() - epoch_start
        epoch_times.append(epoch_seconds)
        if steps == 0:
            raise RuntimeError("B7 completed no training batches inside the runtime budget")
        scheduler.step()
        row = {"epoch": epoch + 1, "loss": loss_sum / steps, "encoder_lr": float(optimizer.param_groups[0]["lr"]),
            "head_lr": float(optimizer.param_groups[1]["lr"]), "epoch_seconds": float(epoch_seconds), "batches": int(steps),
            "study_draws": int(study_draws), "active_supervision_cells_seen": int(active_cells), "positive_cells_seen": int(positive_cells),
            "negative_cells_seen": int(negative_cells), "budget_limited": bool(budget_exhausted)}
        history.append(row)
        print(row)
        torch.save(_checkpoint_payload(model=model, config=config, spec=spec, history=history, b5_checkpoint=b5_path,
            b5_payload=b5_payload, b6_root=Path(b6_root), b6_audit=b6_audit, supervision=supervision,
            target_multiplier=target_multiplier, metadata_stats=metadata_stats, budget=budget), checkpoint_path)
...
def load_b7_checkpoint(checkpoint: str | Path, *, device: torch.device | str = "cpu") -> tuple[KneeMILNet, dict]:
    path = Path(checkpoint)
    if not path.is_file():
    ...
    spec, state = payload.get("model_spec"), payload.get("model_state")
...
def main() -> None:
    ...
    path = train_b7(config, b5_checkpoint=args.b5_checkpoint, b6_root=args.b6_root, out_root=args.out_root)
    print(path)
