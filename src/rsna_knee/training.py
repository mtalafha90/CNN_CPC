...
def train_fold(config: dict, fold: int) -> Path:
    ...
    # Phase A deliberately remains report-only in Stage 2. The fold-local image
    # teacher was generated by a model that can have trained on this inner fold;
    # using it here would contaminate epoch selection even though outer OOF stays clean.
    selection_pseudo, selection_conf, selection_cal = _teacher_arrays(
        df,
        states,
        gold,
        selection_train_mask,
        config,
        fold,
        use_image_teacher=False,
    )
        ...
        json.dumps(
            {
                ...
                "final_image_teacher": bool(config.get("cotrain_stage1_root")),
            },
            indent=2,
        )
    )
